...
class RobotGUI:
    ...
    def __init__(self, args=None):
        ...
        self.buttons_mission = [
            '-MISSION_RUN-', '-MISSION_ENABLE-', '-MISSION_DISABLE-', '-MISSION_STOP-']
    ...
    def create_window(self):
        ...
        icon_path = os.path.join(
            self._script_directory, "img/robot_small_icon.ico")
    ...
    def gui_updater(self):
        while True:
            ...
            self.window.write_event_value(
                '-UPDATE-STATE-MISSION-', state_mission)

            # ----------------------- HUMAN
            new_pose_human = self.human.get_pose()
            self.window.write_event_value(
                '-HUMAN_UPDATE-POSE-', new_pose_human)
    ...
    def tab3_setup(self):
        ...
        left_column = [
            [
                sg.Frame('Mission State', layout=mission_state,  font=(
                    'Ubuntu Mono', 24), background_color='black'),
            ],

            [
                sg.Button('Enable System', key='-MISSION_ENABLE-', size=(16, 3)
                          ), sg.Button('Disable System', key='-MISSION_DISABLE-', size=(16, 3))
            ],

            [
                sg.Button('Run Mission', key='-MISSION_RUN-',
                          size=(34, 3), pad=(6, 0))
            ],

            [
                sg.Button('Emergency Stop', key='-MISSION_STOP-',
                          button_color=('white', 'red'), size=(34, 3), pad=(6, 3))
            ],
        ]

        right_column = [
            [
                sg.Frame('Human State', layout=human_state,  font=(
                    'Ubuntu Mono', 24), background_color='black'),
            ],

            [
                sg.Button('KEYBOARD HUMAN OFF',
                          key='-HUMAN_KEYBOARD-', size=(34, 3))
            ],
        ...
        tab3_layout = [
            [
                sg.Frame('Mission Manager', layout=headers, font=(
                    'Ubuntu Mono', 30), background_color='black')
            ],
    ...
    def astorino_teach_pendant(self, event, values, flag_print_once_astorino, flag_print_running_astorino):
        ...
        # event activated with CONFIRM button
            ...
            if not gamepad_status:  # If Gamepad is off
                ...
                self.window['-A_GAMEPAD_ENABLE-'].update(
                    'GAMEPAD ON', disabled=True)
            else:
                self.astorino_controller.disable_gamepad()
                joy_name = self.astorino_controller.get_gamepad_name()
                self.window['-A_MSG-'].print(f'Disable {joy_name}!')
                self.window['-A_GAMEPAD_ENABLE-'].update(
                    'GAMEPAD OFF', disabled=False)
    ...
    def sawyer_teach_pendant(self, event, values, flag_print_once_sawyer, flag_print_running_sawyer):
        ...
        # event activated with CONFIRM button
            ...
            if not gamepad_status:  # If Gamepad is off
                ...
                self.window['-GAMEPAD_ENABLE-'].update(
                    'GAMEPAD ON', disabled=True)
            else:
    ...
    def mission_callback(self, event, values):
        ...
        if event == '-UPDATE-STATE-MISSION-':
            ...
            if state_mission == 'IDLE':
                ...
                self.window['-STATE-MISSION-'].update(
                    background_color='red', text_color='white')
                self.window['-MISSION_STOP-'].update(
                    'Release E-Stop', disabled=False)
    ...
    def human_control(self, event, values):  # while loop here
        ...
        if state_human == 'SAFE':
            self.window['-STATE-HUMAN-'].update('SAFE',
                                                background_color='green')
        else:
            self.window['-STATE-HUMAN-'].update('DANGER',
        ...
        if event == '-HUMAN_KEYBOARD-':
            ...
            if not keyboard_status:  # If keyboard is off
                ...
                self.human_thread = threading.Thread(
                    target=self.human.keyboard_move)
    ...
    def getori(self, robot):
        ...
        """
        ori = smb.tr2rpy(
            (robot.base.inv() @ robot.fkine(robot.q)).A[0:3, 0:3], order='xyz')
        return ori
